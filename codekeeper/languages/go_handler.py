...
class GoHandler(ILanguageHandler):
    ...
    FUNC_PATTERN = re.compile(
        ...
        r"(?P<name>\w+)\s*\("
        r"(?P<params>[^)]*)\)"
        r"(?:\s*(?P<return_type>\([^)]+\)|[\w\s*]+))?\s*\{",
        re.MULTILINE,
    )

    METHOD_PATTERN = re.compile(
        r"^(?P<indent>\s*)func\s+\((?P<receiver>[\w\s*]+)\)\s*"
        r"(?P<name>\w+)\s*\("
        r"(?P<params>[^)]*)\)"
        r"(?:\s*(?P<return_type>\([^)]+\)|[\w\s*]+))?\s*\{",
        re.MULTILINE,
    ...
    STRUCT_PATTERN = re.compile(
        r"^(?P<indent>\s*)type\s+(?P<name>\w+)\s+struct\s*\{",
        re.MULTILINE,
    )

    INTERFACE_PATTERN = re.compile(
        r"^(?P<indent>\s*)type\s+(?P<name>\w+)\s+interface\s*\{",
        re.MULTILINE,
    )
    ...
    @classmethod
    def from_file(cls, file_path: Path | str) -> "GoHandler":
    ...
    def _parse_function(
        ...
        decorators = []
        if receiver:
            decorators.append(f"receiver: {receiver.strip()}")
    ...
    def _parse_interface(self, match: re.Match, content: str, file_path: Path) -> ClassInfo:
        ...
        methods = self._parse_methods_in_interface(
            interface_content, content, file_path, start_line
        )
    ...
    def _parse_methods_in_struct(
        ...
        methods = []
        struct_lines = struct_content.split("\n")
    ...
    def _parse_methods_in_interface(
        ...
        for i, line in enumerate(interface_lines):
            method_match = self.METHOD_PATTERN.match(line)
            if method_match:
                full_line_start = full_content.find(line)
                if full_line_start != -1:
